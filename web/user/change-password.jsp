...
<%
    ...
    if(!confpass.equals(npass))
    {
        out.println("PASSWORD DOES NOT MATHCH");
    }
    try{ 
    Class.forName("com.mysql.jdbc.Driver");
  String url="jdbc:mysql://localhost:3306/loanbanking";
  Connection con=DriverManager.getConnection(url,"root","root");
  PreparedStatement pst=con.prepareStatement("UPDATE `login` SET `password`=? WHERE `login`.`password` =? and `login`.`email`=?");
